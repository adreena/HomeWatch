function draw_clanedar(){
        ...
        function renderDaysOfMonth(month, year) {
            ...
            d3CalendarGlobals.calendar
             .selectAll("rect")
             .data(daysInMonthToDisplay)
             .attr("id",function (d) {return d[0]+'-'+d3CalendarGlobals.monthToDisplayAsText() + '-' + d3CalendarGlobals.yearToDisplay(); })
        ...
        //***************** this is where I have to load data **********************
        function getDataForMonth() {
            var randomData = [];
            for (var i = 0; i < 35; i++) {
                var temp=[Math.floor(Math.random()*100),Math.floor(Math.random()*100),Math.floor(Math.random()*100),Math.floor(Math.random()*100)];
                randomData.push(temp);
            }
            return randomData;
        }
